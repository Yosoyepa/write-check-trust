...
| `test_transport_reads_pending_output_after_leader_terminates` | `process.x__observe__mutmut_31` | doble COMPLETO y fiel de proceso/selector: registro real de ambos streams del hijo; eventos únicamente para streams registrados; `unregister` retira realmente el stream del registro y `get_map()` refleja el registro vivo (sin mapa artificial); secuencia fija leer-líder → EOF del canal del líder (readiness real, acotada por deadline propio del test (15 s, por encima del presupuesto de 10 s del SUT), sin sleeps) → al retirar el SUT ese canal, el mismo canal queda registrado con los bytes pendientes del descendiente (tubería propia del test escrita y cerrada antes de arrancar) → lectura de esos bytes → su propio EOF → EOF de stderr → fin. Estado realizable: la salida del líder no implica streams cerrados. Oráculo sobre el SUT: `stdout.log` == `leader-done\ndescendant-late\n`, `exit == 0`, `reason == ""`. Historia: v1 sleep(0.3); v2 marcador; v3 pidfd abandonado con diagnóstico (`preflight-h04/pidfd-diag/`); v4 doble con late_fd sin registro y mapa artificial; v5 final: doble completo y fiel. El nombre (`…descendant_output_after_leader_exit`) cambió para no prometer un descendiente real; la integración real de captura y limpieza sigue en las pruebas de transporte existentes |
