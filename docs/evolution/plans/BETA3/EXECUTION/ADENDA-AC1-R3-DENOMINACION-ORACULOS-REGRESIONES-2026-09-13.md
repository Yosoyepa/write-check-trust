...
| `test_transport_reads_pending_output_after_leader_terminates` | `process.x__observe__mutmut_31` | v6 FINAL, doble de Popen con selector real: antes de invocar el SUT se preparan dos pipes reales, se escriben en stdout los bytes esperados (`leader-done\ndescendant-late\n`) y se cierran AMBOS extremos escritores — los buffers contienen salida pendiente sin escritores; el doble de proceso expone esos streams y representa un proceso terminado (`poll()` y `wait()` retornan 0, sin proceso real); `selectors.DefaultSelector` sigue siendo el REAL: el SUT registra sus streams, lee los bytes pendientes y los retira al recibir EOF — sin registrar streams desde unregister ni sustituir canales. Sustitución por el punto existente (`process_transport.subprocess.Popen`). Oráculo: `stdout.log` == `leader-done\ndescendant-late\n`, `exit == 0`, `reason == ""`. Diff exacto del mutante (verificado en el árbol generado): `while selector.get_map() or process.poll() is None:` → `and process.poll() is None` — con `and`, un proceso terminado con salida sin consumir se abandona (`stdout.log` == vacío): representa exactamente la condición descrita. Historia: v1 sleep; v2 marcador; v3 pidfd abandonado con diagnóstico; v4 doble parcial; v5 doble de selector con re-registro; v6 final. La integración real de captura y limpieza sigue en las pruebas de transporte existentes |
