...
| E4 | **GS-2**: `semgrep_scope.py` | **EJECUTADO; regresiones integradas; sensibilidad demostrada 16/16**: bruto **102 killed + 16 survived** (base `ee1e878`); los 16 supervivientes están RECHAZADOS por las regresiones del contrato de diagnóstico aprobado (`GS2-CIERRE-ATRIBUCION-REGRESIONES-2026-09-14.md`, base `79cf65c`). **Hallazgo de instrumento abierto**: parte de los 102 killed no reproduce en sondas frescas (runner in-process de mutmut, contaminación de trampolín) — 35 marks DISCREPANCIA-NO-REPRODUCIDA y los 63 evidenciados requieren revisión ID a ID. Lote NO aprobado como PASS | Revisar el runner (fuera de mutmut) y la atribución ID a ID antes de dar el lote por calificado |
