...
| E4 | **GS-2**: `semgrep_scope.py` | **EJECUTADO; regresiones integradas; sensibilidad demostrada 16/16** (posterior, `ddb1b46`): bruto **102 killed + 16 survived** (base `ee1e878`); 16 supervivientes RECHAZADOS por regresiones. **Diagnóstico de activación** (`GS2-DIAGNOSTICO-ACTIVACION-SONDAS-2026-09-15.md`): defecto del lanzador sellado; hipótesis in-process retirada (mutmut 3.7.0 forkea por mutante). **Revalidación sucesora** (`GS2-REVALIDACION-Y-PREPARACION-CIERRE-2026-09-15.md`): 34/34 IDs abiertos con activación in-process acreditada, control original verde y causa del mutante en fase call; `x__unique_dirs__mutmut_7` promovido por evidencia sellada; 0 discrepancias, 0 timeouts; **atribución pendiente resuelta / sin residuos abiertos bajo evidencia sucesiva** (verifier independiente: APROBAR-EXPEDIENTE CON RESERVAS menores). Final: 63 histórico + 4 adenda + 35 sucesores + 16 supervivientes = 118. No es campaña completa ni PASS de G-MUT/AC1; el lote sigue NO aprobado como PASS | Decisión humana sobre el PASS del lote; GS-3 (E5) planificable tras autorización; presupuesto consumido 102.3 s de 600 s |
| E5 | **GS-3**: `semgrep.py` | Lote PROPUESTO, no ejecutado. **Preparación documental lista** (`ANEXO-PREPARACION-GS3-2026-09-15.md`): alcance `tools/wct/gate/semgrep.py` sha256 `0126c4e1…` (96 líneas, 3 funciones, 51 sitios WCT), precondiciones, receta fail-closed P0–P12, controles C0–C2/N1–N3, paradas y presupuesto estimado (A ≤3600 s; sondas 1800 s estimados). Prompt propuesto, NO ejecutado. Reparación G-DEPS/G-DEAD integrada (`REPARACION-INTEGRACION-GDEPS-GDEAD-2026-09-15.md`): `semgrep.py` conserva `0126c4e1…`; `pyproject.toml` y `uv.lock` cambian de hash respecto a la tabla §1.3 del anexo → re-verificar esa tabla contra el candidato al autorizar (solo BLOQUEA si cambia `semgrep.py`, que no cambió) | Autorización humana + dictamen independiente previo (PROC-005); decisión sobre E4 y pendientes D-A/D-C; congelar candidato al autorizar |
